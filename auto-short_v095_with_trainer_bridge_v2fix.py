...
try:
# === V2 IMPORTS (Step 2) ===
# We split availability flags:
# - V2_CORE_AVAILABLE: engine_v2 + headers (needed for Step 3 timers/UI)
# - V2_SCORING_AVAILABLE: strategy/scoring modules (Step 4+)
...
try:
    ...
    # If even config_v2 can't load — disable V2 features, but keep app running.
    print("[V2][FATAL] Failed to import core.config_v2:", e)
    import traceback as _tb
    print(_tb.format_exc())
    USE_NEW_UI = False
    USE_NEW_FREQUENCIES = False
    USE_NEW_SCORING = False
    USE_NEW_ALGORITHMS = False
...
# Engine + headers are enough for Step 3 (frequencies/timers)
try:
    ...
    print("[V2] core.engine_v2 + ui.table_headers_v2 loaded (Step 3 ready)")
except Exception as e:
    print("[V2] core modules not available:", e)
    import traceback as _tb
    print(_tb.format_exc())
    V2_CORE_AVAILABLE = False

# Scoring/strategy modules (optional; Step 4+)
try:
    from strategies.short_after_pump_v2 import ShortAfterPumpV2, get_strategy as get_strategy_v2
    V2_SCORING_AVAILABLE = True
    print("[V2] strategies.short_after_pump_v2 loaded (Step 4 ready)")
except Exception as e:
    # Not fatal for Step 3 — we can run timers and UI without it.
    V2_SCORING_AVAILABLE = False
    print("[V2] scoring module missing/failed (OK for Step 3):", e)

# Backwards-compatible alias used throughout older code
V2_AVAILABLE = V2_CORE_AVAILABLE
# === END V2 IMPORTS ===
# === END V2 IMPORTS ===
...
class BybitClient:
    ...
    def get_top200_turnover(self, top_n: int = 200, *, ttl_sec: int = 600) -> List[Dict]:
        ...
        # ttl_sec <= 0 => no cache (force fresh)
        cache_key = f"top_tickers_{top_n}"
        try:
            if self._tickers_cache is not None:
                exp_ts, key, cached = self._tickers_cache
                if ttl_sec > 0 and key == cache_key and exp_ts > time.time() and isinstance(cached, list) and cached:
                    return cached
        except Exception:
            pass
...
class MainWindow(QMainWindow):
    def __init__(self, client: BybitClient, tg_token: str, tg_chat_id: Optional[int], tg_thread_id: Optional[int], tg_lev: int):
        ...
        # V2 Step 3: UI progress-bars refresh (shows real remaining seconds)
        self._v2_bars_timer: Optional[QTimer] = None
        if V2_AVAILABLE and USE_NEW_FREQUENCIES:
            try:
                self._v2_bars_timer = QTimer(self)
                self._v2_bars_timer.setInterval(500)  # twice per second
                self._v2_bars_timer.timeout.connect(self._update_v2_progress_bars)
                self._v2_bars_timer.start()
                print("[V2] progress-bars timer started: 500ms")
            except Exception as e:
                self._v2_bars_timer = None
                print("[V2][WARN] progress-bars timer failed:", e)

        # V2 Step 3: Level-2 Prices timer (TOP200 price refresh every 30s)
        self._level2_prices_timer: Optional[QTimer] = None
        if V2_AVAILABLE and USE_NEW_FREQUENCIES:
            try:
                self._level2_prices_timer = QTimer(self)
                self._level2_prices_timer.setInterval(int(SCAN_LEVEL2_INTERVAL_SEC * 1000))
                self._level2_prices_timer.timeout.connect(self._on_level2_prices_tick_safe)
                self._level2_prices_timer.start()
                print(f"[V2] level2 prices timer started: {SCAN_LEVEL2_INTERVAL_SEC}s")
            except Exception:
                self._level2_prices_timer = None
    ...
    def start_scan(self):
        ...
        # V2: start Level-1 (full) cycle marker
        if V2_AVAILABLE and USE_NEW_FREQUENCIES:
            try:
                mark_level_started(1)
            except Exception:
                pass
    ...
    def _update_v2_progress_bars(self):
        ...
        try:
            ...
            # 1) Timers
            update_scan_timers()
            st = get_scan_state()  # ScanState dataclass

            header_mgr = get_header_manager()
            header_mgr.update_level_timers(
                level1_remaining=int(getattr(st, 'level1_remaining_sec', 0) or 0),
                level2_remaining=int(getattr(st, 'level2_remaining_sec', 0) or 0),
                level3_remaining=int(getattr(st, 'level3_remaining_sec', 0) or 0),
                candidates_remaining=int(getattr(st, 'level3_remaining_sec', 0) or 0),
            )
            header_mgr.set_level_in_progress(1, bool(getattr(st, 'level1_in_progress', False)))
            header_mgr.set_level_in_progress(2, bool(getattr(st, 'level2_in_progress', False)))
            header_mgr.set_level_in_progress(3, bool(getattr(st, 'level3_in_progress', False)))

            # 2) Counts (best-effort; do not break UI if some fields are absent)
            try:
                # Criteria type lives in V2 metrics; if it's not present yet -> zeros
                classic = pump5m = pump1m = combo = 0
                selected = 0
                for _sym, r in self.rows_by_symbol.items():
                    if not _sym.endswith('USDT'):
                        continue
                    selected += 1
                    ct = str(getattr(r, 'criteria_type', '') or '')
                    if ct == 'CLASSIC':
                        classic += 1
                    elif ct == 'PUMP_5M':
                        pump5m += 1
                        ...
                        combo += 1
                header_mgr.update_counts(selected=selected, classic=classic, pump_5m=pump5m, pump_1m=pump1m, combo=combo)
            except Exception:
                pass

            # 3) Candidate status counts (derived from current focus list when available)
            try:
                candidates = getattr(self, '_last_focus', []) or []
                watch = sum(1 for c in candidates if str(c.get('status', '')) == 'Наблюдение')
                interest = sum(1 for c in candidates if str(c.get('status', '')) == 'Интерес')
                ready = sum(1 for c in candidates if str(c.get('status', '')) == 'Готовность')
                entry = sum(1 for c in candidates if str(c.get('status', '')) == 'ВХОД')
                header_mgr.update_status_counts(watch=watch, interest=interest, ready=ready, entry=entry)
            except Exception:
                pass

            # 4) Render header texts
            try:
    ...
    def on_done_all(self, rows_map: dict):
        ...
        # V2: complete Level-1 marker
        if V2_AVAILABLE and USE_NEW_FREQUENCIES:
            try:
                mark_level_complete(1)
            except Exception:
                pass
    ...
    def on_focus_tick(self):
        ...
        """
        try:
            # V2: treat this tick as Level-3 (detector cadence)
            if V2_AVAILABLE and USE_NEW_FREQUENCIES:
                try:
                    mark_level_started(3)
                except Exception:
                    pass
...
# Мы вернёмся к стакану позже отдельным шагом, когда потребуется.: Metrics toggle --------------------
    ...
    # ---------------- V2: Level-2 prices tick (TOP200) ----------------
    def _on_level2_prices_tick_safe(self):
        """Safe wrapper for the 30s TOP200 price refresh.

        IMPORTANT: Always print traceback to terminal + write to logs/errors.log.
        """
        try:
            self._on_level2_prices_tick()
        except Exception as e:
            try:
                log_error("level2_prices_tick", e)
            except Exception:
                pass
            try:
                print(f"[ERROR] level2_prices_tick: {e}")
                print(traceback.format_exc())
            except Exception:
                pass

    def _on_level2_prices_tick(self):
        """Refresh TOP200 tickers data (prices/high/low/24h) every 30 seconds.

        IMPORTANT: this must NOT run heavy klines; only market/tickers batch.
        """
        if not (V2_AVAILABLE and USE_NEW_FREQUENCIES):
            return
        # Do not interfere with full scan (worker) to avoid flicker and data races
        if getattr(self, "scan_in_progress", False):
            return
        try:
            mark_level_started(2)
        except Exception:
            pass
        self._refresh_top200_prices()
        # Re-sort in base mode so that table is truly dynamic
        try:
            symbols = [s for s in self.rows_by_symbol.keys() if s.endswith("USDT") and s not in PINNED]
            self.sort_mode = "base"
            self.sorted_symbols = self.sorted_by_current_sort(symbols)
        ...
        self.render_main_table()
        try:
            mark_level_complete(2)
        except Exception:
            pass

    def _refresh_top200_prices(self) -> None:
        """Updates rows_by_symbol fields from latest tickers (1 batched request)."""
        if not self.rows_by_symbol:
            return
        try:
            tickers = self.client.get_top200_turnover(200, ttl_sec=0)
        except Exception:
            return
        if not tickers:
            return
        mp = {str(t.get("symbol", "")).upper(): t for t in tickers}
        ...
        for sym, r in list(self.rows_by_symbol.items()):
            ...
            change_raw = fnum(t.get("price24hPcnt", 0.0))
            turnover = fnum(t.get("turnover24h", 0.0))
            funding = fnum(t.get("fundingRate", 0.0))

            if px > 0:
                r.price_now = px
            r.high_24h = high_24h
            r.low_24h = low_24h
            r.change_24h_pct = change_raw * 100.0
            r.funding_rate = funding
            r.turnover_24h = turnover
            r.vol24_m = int(turnover / 1e6)

            rng = high_24h - low_24h
            if rng > 0 and px > 0:
                r.range_position = ((px - low_24h) / rng) * 100.0
            else:
                r.range_position = getattr(r, "range_position", 50.0) or 50.0

            if high_24h > 0 and px > 0:
                r.dist_to_high_pct = ((high_24h - px) / high_24h) * 100.0
            else:
                r.dist_to_high_pct = getattr(r, "dist_to_high_pct", 0.0) or 0.0
...
def main() -> int:
    ...
    Returns exit code (0=ok). Any exception is logged + printed, then returns 1.
    """
    print("[DEBUG-MAIN] ==========================================")
    ...
    try:
        ...
        try:
            print(f"[FATAL] app.exec() crashed: {e}")
            print(traceback.format_exc())
        except Exception:
            pass
        return 1
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            ...
            pass
        try:
            print(f"[FATAL] __main__ crashed: {e}")
            print(traceback.format_exc())
        except Exception:
            pass
        exit_code = 1
    finally:
        # IMPORTANT: terminal must not close — show traceback & wait
